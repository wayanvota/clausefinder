...
if (!meta.totalNodes) {
  throw new Error("FAR index is empty. Run npm --prefix backend run index:far.");
}

const sam = await searchFar({ query: "What changed about SAM registration?", limit: 5 });
...
const sensitive = await searchFar({ query: "CUI source selection proposal with proprietary price" });
